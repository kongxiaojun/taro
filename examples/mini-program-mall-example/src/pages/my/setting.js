...
@withWeapp({
  data: {
    enableDebug: Taro.getSystemInfoSync().enableDebug,
  },
  ...
  goadmin() {
    Taro.navigateToMiniProgram({
      ...
      path: 'pages/login/auto?token=' + Taro.getStorageSync('token'),
      envVersion: 'trial', // develop trial release
    })
  },

  setEnableDebug() {
    const enableDebug = Taro.getSystemInfoSync().enableDebug
    if (enableDebug) {
      Taro.setEnableDebug({
        enableDebug: false,
      })
...
class _C extends React.Component {
  render() {
    ...
    return (
      <Block>
        {apiUserInfoMap && <View className="header-box">
        ...
        {fx_type == 'hehuoren' ? (
          ...
          show_3_seller == 1 && apiUserInfoMap && (
