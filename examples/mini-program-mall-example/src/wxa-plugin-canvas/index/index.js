  ...
  _drawSingleText({
    ...
    textArr.forEach((item, index) => {
      this.ctx.fillText(
        item,
        this.toPx(x),
        this.toPx(y + (lineHeight || fontSize) * index)
      )
    })
    this.ctx.restore()
...
const helper = {
  ...
  _downImage(imageUrl) {
    return new Promise((resolve, reject) => {
      if (
        /^http/.test(imageUrl) &&
        !new RegExp(Taro.env.USER_DATA_PATH).test(imageUrl)
      ) {
        Taro.downloadFile({
...
class _C extends React.Component {
  render() {
    const { debug, pxWidth, pxHeight } = this.data
    return (
      <View className="container">
        <Canvas
          canvasId="canvasid"
          className={'canvas ' + (debug ? 'debug' : 'pro')}
