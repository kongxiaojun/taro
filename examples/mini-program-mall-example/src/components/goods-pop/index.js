  ...
  methods: {
    ...
    skuSelect(e) {
      ...
      for (
        ...
        index < skuCurGoods.properties.length;
        index++
      ) {
    ...
    async calculateGoodsPrice() {
      ...
      // 计算 sku 价格
      const needSelectNum = this.data.skuCurGoods.properties
        ? this.data.skuCurGoods.properties.length
        : 0
      let curSelectNum = 0
      ...
      // 计算可选配件
      if (
        this.data.skuCurGoods.basicInfo.hasAddition &&
        this.data.goodsAddition
      ) {
    ...
    return (
      ...
      >
        ...
        <View className="sku-container">
          {skuCurGoods && skuCurGoods.properties.map((item, idx) => {
            return (
              <View className="sku" key={item.id}>
                <View className="t">{item.name}</View>
                <View className="items">
                  {item.childsCurGoods.map((small, index) => {
                    return (
                      <Text
          ...
          {goodsAddition && goodsAddition.map((item, idx) => {
            return (
              <View className="sku" key={item.id}>
                <View className="t">{item.name}</View>
                <View className="items">
                  {item.items.map((small, index) => {
                    return (
                      <Text
                        className={small.active ? 'active' : ''}
                        key={small.id}
          ...
          <View className="num">
            <View className="t">购买数量</View>
            {skuCurGoods && <VanStepper
              value={skuCurGoods.basicInfo.storesBuy}
              onPlus={this.storesJia}
              onMinus={this.storesJian}
            ></VanStepper>}
          </View>
        </View>
        <View className="btn">
          <VanButton type="danger" block round onClick={this.addCarSku}>
            加入购物车
          </VanButton>
        </View>
      </VanPopup>
